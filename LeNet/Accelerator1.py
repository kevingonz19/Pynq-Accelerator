...
class Convolution2D:
    def __init__(self, Input_planes, Out_planes, Conv_H, Conv_W, Stride_H, Stride_W, Padding, Relu, Weight,
                 Bias=np.any([False]), Precision=10000, executor=None):
    ...
    async def forward(self, data, dma, Load_Input=1, Load_Weight=1):
        ...
        await asyncio.get_event_loop().run_in_executor(self.executor, dma.sendchannel.transfer, Configuration)
        await asyncio.get_event_loop().run_in_executor(self.executor, dma.sendchannel.wait)
        await asyncio.get_event_loop().run_in_executor(self.executor, dma.recvchannel.transfer, Configuration)
        await asyncio.get_event_loop().run_in_executor(self.executor, dma.recvchannel.wait)
        ...
        for idx in range(int((x.shape[0]) / 3000)):
            ...
            await asyncio.get_event_loop().run_in_executor(self.executor, dma.sendchannel.transfer, in_buffer)
            await asyncio.get_event_loop().run_in_executor(self.executor, dma.sendchannel.wait)
            await asyncio.get_event_loop().run_in_executor(self.executor, dma.recvchannel.transfer, in_buffer)
            await asyncio.get_event_loop().run_in_executor(self.executor, dma.recvchannel.wait)
        if ((x.shape[0]) % 3000 != 0):
            ...
            await asyncio.get_event_loop().run_in_executor(self.executor, dma.sendchannel.transfer, in_buffer_remind)
            await asyncio.get_event_loop().run_in_executor(self.executor, dma.sendchannel.wait)
            await asyncio.get_event_loop().run_in_executor(self.executor, dma.recvchannel.transfer, in_buffer_remind)
            await asyncio.get_event_loop().run_in_executor(self.executor, dma.recvchannel.wait)
...
class Pool:
    def __init__(self, Win_H, Win_W, Stride_H, Stride_W, Pool_type, Padding, Relu, Precision=10000, executor=None):
        self.Win_H = Win_H
        self.Win_W = Win_W
        self.Stride_H = Stride_H
        self.Stride_W = Stride_W
    ...
    async def forward(self, data, dma, Load_Input=1):
...
class FC:
    ...
    def __init__(self, Input_Size, Output_Size, Relu, Weight, Bias=np.any([False]), Precision=10000, executor=None):
